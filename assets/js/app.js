...
let /** {string} */ apiUrl = 'https://api.github.com/users/Basavarajakjbasuu';
  ...
  fetchData(profileURL, data => {
    ...
    $profileCard.innerHTML = `
      ...
      <a href="${githubPage}" target="_blank" class="btn btn-secondary">
        ...
        <span class="span">See in Github</span>
      </a> 
...
const /** {NodeElement} */ $followerTabBtn = document.querySelector('[ data-followers-tab-btn]');
  ...
  fetchData(followerUrl, function (data) {
    ...
    if (data.length) {
      for (const item of data) {
        ...
        $followerPanel.appendChild($followingCard)
      }
    } else {
      $followerPanel.innerHTML = `
        <div class="error-content">
